...
const state = {auth:null,departments:[],users:[],roles:[],tenants:[],localAccounts:[],selectedRole:'',selectedTenant:'',search:''};
const elements = {
    ...
    tenantManagement:document.getElementById('tenantManagement'),tenantList:document.getElementById('tenantList'),selectedTenantName:document.getElementById('selectedTenantName'),selectedTenantMeta:document.getElementById('selectedTenantMeta'),tenantLoginAddress:document.getElementById('tenantLoginAddress'),tenantLoginUrl:document.getElementById('tenantLoginUrl'),copyTenantLoginBtn:document.getElementById('copyTenantLoginBtn'),newTenantBtn:document.getElementById('newTenantBtn'),syncTenantProfileBtn:document.getElementById('syncTenantProfileBtn'),syncTenantBtn:document.getElementById('syncTenantBtn'),deleteTenantBtn:document.getElementById('deleteTenantBtn'),inviteDialog:document.getElementById('inviteDialog'),inviteTenantName:document.getElementById('inviteTenantName'),inviteExpires:document.getElementById('inviteExpires'),generateInviteBtn:document.getElementById('generateInviteBtn'),inviteResult:document.getElementById('inviteResult'),inviteUrl:document.getElementById('inviteUrl'),copyInviteBtn:document.getElementById('copyInviteBtn'),inviteExpiry:document.getElementById('inviteExpiry'),
    rolesTab:document.getElementById('rolesTab'),externalTab:document.getElementById('externalTab'),externalForm:document.getElementById('externalAccountForm'),externalRows:document.getElementById('externalAccountRows'),externalSummary:document.getElementById('externalSummary'),externalTenant:document.getElementById('externalTenant'),externalUsername:document.getElementById('externalUsername'),externalDisplayName:document.getElementById('externalDisplayName'),externalPassword:document.getElementById('externalPassword'),externalRole:document.getElementById('externalRole'),externalExpiresAt:document.getElementById('externalExpiresAt'),
};
...
function isPlatformAdmin(){return Boolean(state.auth?.user?.is_super_admin);}
...
function roleLabel(role){return role.name==='admin'?'企业管理员':(role.display_name||role.name);}
function roleOptions(selected,allowAdmin=isPlatformAdmin()){return state.roles.filter(role=>role.name!=='admin'||allowAdmin||role.name===selected).map(role=>`<option value="${escapeHtml(role.name)}" ${role.name===selected?'selected':''}>${escapeHtml(roleLabel(role))}</option>`).join('');}
...
function renderExternal(){if(!elements.externalRows)return;const selectedTenant=state.selectedTenant||state.auth?.user?.tenant_key||'';const tenants=state.tenants.length?state.tenants:[{tenant_key:selectedTenant,name:selectedTenant}];const tenantNames=new Map(tenants.map(item=>[item.tenant_key,item.name||item.tenant_key]));elements.externalTenant.innerHTML=tenants.map(item=>`<option value="${escapeHtml(item.tenant_key)}" ${item.tenant_key===selectedTenant?'selected':''}>${escapeHtml(item.name||item.tenant_key)}</option>`).join('');elements.externalRole.innerHTML=roleOptions(elements.externalRole.value||'guest');if(!elements.externalExpiresAt.value)elements.externalExpiresAt.value=defaultExpiryDate();elements.externalSummary.textContent=`${state.localAccounts.length} 个账号`;elements.externalRows.innerHTML=state.localAccounts.map(account=>`<tr data-account-id="${account.id}"><td>${escapeHtml(account.display_name)}<small>${escapeHtml(account.username)}</small></td><td>${escapeHtml(tenantNames.get(account.tenant_key)||account.tenant_key)}</td><td><select data-field="role">${roleOptions(account.role,true)}</select></td><td><select data-field="status"><option value="active" ${account.status==='active'?'selected':''}>启用</option><option value="disabled" ${account.status==='disabled'?'selected':''}>停用</option></select></td><td><input data-field="expires-at" type="date" value="${localDateValue(account.expires_at)}"></td><td><div class="external-row-actions"><button type="button" data-action="save">保存</button><button type="button" data-action="delete">删除</button></div></td></tr>`).join('');}
function renderTenantManagement(){if(!elements.tenantManagement)return;const visible=Boolean(state.auth?.user?.is_super_admin);elements.tenantManagement.hidden=!visible;if(!visible)return;const tenant=state.tenants.find(item=>item.tenant_key===state.selectedTenant);elements.tenantList.innerHTML=state.tenants.map(item=>`<button class="tenant-list-item ${item.tenant_key===state.selectedTenant?'active':''}" type="button" data-tenant="${escapeHtml(item.tenant_key)}"><span><strong>${escapeHtml(item.name||'未命名企业')}</strong><small>${item.credential_configured?'已完成飞书认证':'待完成飞书认证'}</small></span><i class="tenant-auth-state ${item.credential_configured?'verified':'pending'}">${item.credential_configured?'已认证':'待认证'}</i></button>`).join('');elements.selectedTenantName.textContent=tenant?.name||'请选择企业';elements.selectedTenantMeta.textContent=tenant?`${tenant.credential_configured?'已完成飞书认证':'待完成飞书认证'} · ${state.users.length} 名成员`:'';const loginPath=tenant?.slug?`/t/${encodeURIComponent(tenant.slug)}/login`:'';const loginUrl=loginPath?`${location.origin}${loginPath}`:'';elements.tenantLoginUrl.textContent=loginUrl;elements.tenantLoginAddress.hidden=!loginUrl;elements.deleteTenantBtn.disabled=!tenant||state.tenants.length<=1;elements.syncTenantProfileBtn.disabled=!tenant||!tenant.credential_configured;elements.syncTenantBtn.disabled=!tenant||!tenant.credential_configured;}
function renderMembers(){
    ...
    elements.memberRows.innerHTML=users.map(user=>{const id=user.open_id||user.user_id||'';const names=(user.department_ids||[]).map(item=>departments.get(String(item))||item).filter(Boolean).join('、')||'未分配部门';const platformAdmin=Boolean(user.is_super_admin);const tenantAdmin=!platformAdmin&&user.role==='admin';const editable=canManage()&&!platformAdmin&&(isPlatformAdmin()||!tenantAdmin);const disabled=editable?'':'disabled';const identitySuffix=platformAdmin?'（平台超管）':(tenantAdmin?'（企业管理员）':'');const displayName=`${user.name||'未命名成员'}${identitySuffix}`;const action=user.managed&&editable?'<button class="remove-command" type="button" data-action="remove" title="移除单独角色覆盖，恢复默认 member 权限">恢复默认</button>':`<span class="assignment-state" title="${tenantAdmin?'管理员角色只能由平台超管调整':'该成员使用默认 member 权限'}">${tenantAdmin?'管理员':(user.managed?'已授权':'默认成员')}</span>`;return `<tr data-user-id="${escapeHtml(id)}"><td><div class="member-name">${escapeHtml(displayName)}</div><div class="member-sub">${escapeHtml(user.email||user.mobile||id)}</div></td><td><div class="department-list">${escapeHtml(names)}</div></td><td><select data-field="role" ${disabled}>${roleOptions(user.role)}</select></td><td><select data-field="status" ${disabled}><option value="active" ${user.status!=='disabled'?'selected':''}>启用</option><option value="disabled" ${user.status==='disabled'?'selected':''}>停用</option></select></td><td>${action}</td></tr>`;}).join('');
}
function renderRoleList(){elements.roleList.innerHTML=state.roles.map(role=>`<button class="role-item ${role.name===state.selectedRole?'active':''}" type="button" data-role="${escapeHtml(role.name)}"><span>${escapeHtml(roleLabel(role))}</span><span class="role-key">${escapeHtml(role.name)}</span></button>`).join('');}
function selectedRole(){return state.roles.find(role=>role.name===state.selectedRole)||null;}
function permissionOptions(items,selected){return items.map(([key,label])=>{const disabled=!isPlatformAdmin();return `<label class="permission-option"><input type="checkbox" value="${escapeHtml(key)}" ${selected.has(key)?'checked':''} ${disabled?'disabled':''}><span>${escapeHtml(label)}</span></label>`;}).join('');}
...
function renderRoleEditor(){const role=selectedRole();const selected=new Set(role?.permissions||[]);elements.roleName.value=role?.name||'';elements.roleDisplayName.value=role?.display_name||'';elements.roleName.disabled=!isPlatformAdmin()||Boolean(role);elements.roleDisplayName.disabled=!isPlatformAdmin();elements.saveRoleBtn.disabled=!isPlatformAdmin();elements.newRoleBtn.disabled=!isPlatformAdmin();if(elements.deleteRoleBtn)elements.deleteRoleBtn.disabled=!isPlatformAdmin()||!role||Boolean(role.system);if(elements.roleScopeNote){elements.roleScopeNote.textContent='当前为平台超管：平台超管始终拥有全部权限；角色修改只影响被分配该角色的企业成员。';elements.roleScopeNote.hidden=!role;}elements.permissionGroups.innerHTML=menuPermissionGroups(selected)+permissionGroup('操作能力',CAPABILITY_PERMISSIONS,selected);}
function renderAll(){renderTenantManagement();renderMembers();renderRoleList();renderRoleEditor();renderExternal();const platformAdmin=isPlatformAdmin();if(elements.rolesTab)elements.rolesTab.hidden=!platformAdmin;if(elements.externalTab)elements.externalTab.hidden=!platformAdmin||!state.auth?.config?.local_auth_enabled;if(!platformAdmin){document.querySelectorAll('.tab').forEach(tab=>tab.classList.toggle('active',tab.dataset.tab==='members'));document.querySelectorAll('.panel').forEach(panel=>panel.classList.toggle('active',panel.id==='membersPanel'));}}
...
async function loadData(reload=false){
    ...
    try{
        ...
        const organizationUrl=scopedUrl('/api/admin/organization')+(reload?`${state.selectedTenant?'&':'?'}_=${Date.now()}`:'');
        const [data,accountData]=await Promise.all([
            api(organizationUrl),
            isPlatformAdmin()&&state.auth?.config?.local_auth_enabled?api(scopedUrl('/api/admin/local-accounts')):Promise.resolve({accounts:[]}),
        ]);
        state.departments=data.departments||[];state.users=data.users||[];state.roles=data.roles||[];
        state.localAccounts=accountData.accounts||[];
        ...
        else if(!canManage())showNotice('当前账号仅可查看组织架构，成员授权由企业管理员维护。');
        renderAll();
...
async function saveMember(row){
    ...
    try{const saved=await api(scopedUrl(`/api/admin/members/${encodeURIComponent(id)}`),{method:'PUT',body:JSON.stringify({...user,role,status})});user.role=saved.role||role;user.status=saved.status||status;user.is_tenant_admin=user.role==='admin'&&!user.is_super_admin;user.managed=Boolean(user.role!=='member'||user.status!=='active');renderMembers();showToast('成员授权已保存');}
...
async function saveRole(){
    ...
    catch(error){showToast(error.message||String(error),true);}finally{elements.saveRoleBtn.disabled=!isPlatformAdmin();}
}
async function deleteRole(){
    const role=selectedRole();if(!role||role.system||!isPlatformAdmin())return;
...
async function syncTenantProfile(){const key=state.selectedTenant;if(!key)return;elements.syncTenantProfileBtn.disabled=true;try{const result=await api(`/api/admin/tenants/${encodeURIComponent(key)}/sync-profile`,{method:'POST'});showToast(`企业名称已更新为“${result.name}”`);await loadData();}catch(error){showToast(error.message||String(error),true);}finally{elements.syncTenantProfileBtn.disabled=false;}}
...
elements.memberRows.addEventListener('change',event=>{const row=event.target.closest('tr[data-user-id]');if(!row||!event.target.matches('select[data-field]'))return;saveMember(row);});
...
elements.externalTenant?.addEventListener('change',()=>{state.selectedTenant=elements.externalTenant.value;loadData(true);});
elements.externalForm?.addEventListener('submit',async event=>{event.preventDefault();const submit=event.target.querySelector('button[type="submit"]');submit.disabled=true;try{const account=await api('/api/admin/local-accounts',{method:'POST',body:JSON.stringify({tenant_key:elements.externalTenant.value,username:elements.externalUsername.value,display_name:elements.externalDisplayName.value,password:elements.externalPassword.value,role:elements.externalRole.value,expires_at:elements.externalExpiresAt.value})});state.localAccounts.push(account);event.target.reset();elements.externalTenant.value=account.tenant_key;elements.externalRole.value=account.role;elements.externalExpiresAt.value=defaultExpiryDate();renderExternal();showToast('外部账号已创建');}catch(error){showToast(error.message||String(error),true);}finally{submit.disabled=false;}});
elements.externalRows?.addEventListener('click',async event=>{const button=event.target.closest('[data-action]');const row=button?.closest('tr[data-account-id]');if(!button||!row)return;const accountId=row.dataset.accountId;const action=button.dataset.action;if(!['save','delete'].includes(action))return;row.querySelectorAll('button,select,input').forEach(control=>{control.disabled=true;});try{if(action==='delete'){await api(scopedUrl(`/api/admin/local-accounts/${accountId}`),{method:'DELETE'});state.localAccounts=state.localAccounts.filter(item=>String(item.id)!==accountId);}else{const payload={role:row.querySelector('[data-field="role"]').value,status:row.querySelector('[data-field="status"]').value,expires_at:row.querySelector('[data-field="expires-at"]').value};const account=await api(scopedUrl(`/api/admin/local-accounts/${accountId}`),{method:'PATCH',body:JSON.stringify(payload)});const index=state.localAccounts.findIndex(item=>String(item.id)===String(account.id));if(index>=0)state.localAccounts[index]=account;}renderExternal();showToast(action==='delete'?'外部账号已删除':'外部账号已保存');}catch(error){renderExternal();showToast(error.message||String(error),true);}});
